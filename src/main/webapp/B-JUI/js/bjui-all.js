...
+function ($) {
    ...
    Sidenav.prototype.ajaxHnav = function() {
        ...
        var createA = function(data) {
            ...
            if (data.url && (target === 'dialog' || target === 'navtab')) {
                liHtml.add('data-toggle="'+ target +'" data-options="{id:\''+ data.id +'\', title:\''+ (data.title || data.name) +'\', icon:\'' + data.icon+'\'')
    ...
    Sidenav.prototype.initHnav = function() {
        ...
        var createA = function(data) {
            ...
            if (data.url && (target === 'dialog' || target === 'navtab')) {
                liHtml.add('data-toggle="'+ target +'" data-options="{id:\''+ data[idKey] +'\', title:\''+ (data.title || data.name)  +'\', icon:\'' + data.icon+'\'')
...
+function ($) {
    ...
    Navtab.prototype.openTab = function() {
        ...
        if (iOpenIndex >= 0) {
            ...
            var tabFrag = '<li><a href="javascript:" title="#title#"><i class="fa fa-#icon#"></i>&nbsp;<span>#title#</span></a><span class="close">&times;</span></li>',
                $tab = $(tabFrag.replaceAll('#title#', options.title).replaceAll('#icon#', options.icon)),
...
+function ($) {
    ...
    Datagrid.prototype.init = function() {
        ...
        var toolbarTitle = options.gridTitle ? '<div class="datagrid-toolbar-title">'+ options.gridTitle +'</div>' : '';
        gridHtml
            .add('<div class="bjui-datagrid">')
            //.add(options.gridTitle ? '<div class="datagrid-title">'+ options.gridTitle +'</div>' : '')
            .add(options.showToolbar ? '<div class="datagrid-toolbar">' + toolbarTitle + '</div>' : '')
