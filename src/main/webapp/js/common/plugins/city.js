...
// cityId:市select id, countyId:区select id, isSelectpicker:true初始化后会刷新
function citySelect(cityId, countyId, isSelectpicker) {
	var cityJson = [
	...
	if (cityId != null && cityId != "") {
		...
		initSelect(cityId, cityHtml);

		$(provId).change(function(){
			var cityHtml = "<option value=''>请选择市</option>";
			$.each(cityJson,
					function(i, val) {
				if (val.item_code.substr(0, 2) == $(provId).val().substr(0, 2) && val.item_code.substr(2, 4) != '0000' && val.item_code.substr(4, 2) == '00') {
					cityHtml += "<option value='" + val.item_code + "'>" + val.item_name + "</option>";
				}
			});
			initSelect(cityId, cityHtml);
		});
	...
	if (countyId != null && countyId != "") {
		...
		initSelect(countyId, countyHtml);
			...
			$.each(cityJson,
				...
				if (cityVal == '110100' || cityVal == "120100" || cityVal == "310100" || cityVal == "500100") {
					if (val.item_code.substr(0, 3) == cityVal.substr(0, 3) && val.item_code.substr(4, 2) != '00') {
						countyHtml += "<option value='" + val.item_code + "'>" + val.item_name + "</option>";
					}
				} else {
	...
	var provHtml= "<option value=''>请选择省</option>";
	$.each(cityJson,
			function(i, val) {
		if (val.item_code.substr(2, 4) == '0000') {
			provHtml += "<option value='" + val.item_code + "'>" + val.item_name + "</option>";
		}
	});
	initSelect(provId, provHtml);
}